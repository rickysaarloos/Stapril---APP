 ...
 * @param {{heeftTeam?: boolean, heeftFoto?: boolean, heeftNaam?: boolean, aantalBadges?: number}} profiel
...
function berekenVerdiendeBadges(stepsMap, profiel = {}) {
  ...
  for (let i = 1; i < datums.length; i++) {
    ...
    // Alleen als ze opeenvolgende kalenderdagen zijn
  ...
  for (const zat of zaterdagen) {
    const weekNr = Math.floor(new Date(zat).getTime() / (7 * 86400000))
    const heeftZondag = zondagen.some(zo => {
      const diff = (new Date(zo) - new Date(zat)) / 86400000
      return diff === 1 // zondag direct na zaterdag
    })
  ...
  for (let i = 1; i < maandagenMetDoel.length; i++) {
    ...
    if (diff === 7) {
      maReeks++
      if (maReeks >= 2) { verdiend.add('vroege_vogel'); break } // 3 maandagen = 2 overgangen
    } else {
      maReeks = 0
    }
  }
...
export async function verwerkBadges(uid, stepsMap, profiel = {}) {
  // Haal al bestaande badges op voor de badgejager-telling
  const bestaandeSnap = await getDocs(collection(db, 'users', uid, 'badges'))
  const aantalBadges = bestaandeSnap.size
 
  const verdiend = berekenVerdiendeBadges(stepsMap, { ...profiel, aantalBadges })
  const nieuw = []
